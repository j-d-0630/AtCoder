N = int(input())
S = input()
# dp = [[インクリメント用,aまで合ってる物の数,atまで合ってるの物の数,atcまで合ってる物の数,atco,atcor,atcord,atcorde,atcorderまで合ってる物の数],[],[],[]]
# インクリメント用に1、atcorderで7の計8要素 × 入力最大文字数100000の配列となる
...
for i in range(N): #全ての入力文字を順番に評価
  for j in range(8):
    ...
    if S[i] == "r" and j == 6: dp[i+1][j+1] += dp[i][j] # rが出現したときは、これまでの評価でatcordeまで合っている物の数を加える
  for k in range(8): # 毎回割って余りにしておく
    dp[i+1][k] %= mod

# 最後の文字を評価しatcorderまで合っている物の数は、dp[N][7]に格納されている
print(dp[N][7])
